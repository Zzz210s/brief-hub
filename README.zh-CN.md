...
简报类型统一为 `change`,在会话**改动了某个文件夹里的文件**时投出(或发生任务级失败时);只跑命令不再产生简报。

- 标题:`变更 <文件夹> · N 个文件`(推送过则附 `· 已推送`)
- 标签:`dir:<文件夹>` —— 可按文件夹订阅,如 `bh sub add dir:src --sess=<id>`
- 严重度仍区分信号:`info`(普通变更)、`warn`(工具级失败)、`err`(任务级失败)
